...
function draw() {
   ...
   for (let f of fireworks) {
      if (f.active) {
         ...
         if (abs(f.x - mouseX) + abs(f.y - mouseY) < 20) {
            if (dist(f.x, f.y, mouseX, mouseY) < 5 + f.size / 2) {
               alive = false;
               millisLost = millis();
   ...
   for (let p of particles) {
      if (p.active) {
         ...
         if (abs(p.x - mouseX) + abs(p.y - mouseY) < 50) {
            if (dist(p.x, p.y, mouseX, mouseY) < 5 + p.size / 2) {
               alive = false;
               millisLost = millis();
   ...
   if (key == 'd') {
      ...
      text("Active particles: " + activeParticles, 0, height - 10);
      ...
      text("Active fireworks: " + activeFireworks, 0, height - 60);
   }
